...
public class InscriptionAssuranceVieService {
	...
	public int save(Inscription ins) {
		try {
		...
		ins.getAssurance().setDateInscription(date);
		InscriptionAssuranceVie inscription = iAVDao.save(ins.getAssurance());
		Double total=0.0;
		for(InscriptionAssuranceVieProduitFinancier iProduit : ins.getProduits()) {
